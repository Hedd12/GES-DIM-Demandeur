...
app.post('/register', async (req, res) => {
    ...
    if (errors.length > 0) {
        return res.render('register', { errors }); // Changé de 'connexion' à 'register'
    }
    ...
    try {
        ...
        // Insérer le patient avec le même format que les autres requêtes
    ...
    `${process.env.URL_WEBSERVICE}ExecStoredProc/InsertPatient/${encodeURIComponent(insert)}`,
    {
        ...
        // console.log("etape1.5)");
        // // if (!patientResponse.data?.result?.[0]?.id) {
        // //     throw new Error('Échec de la création du patient : ID non retourné');
        // // }
        // const patientId = patientResponse.data.result[0].id;
        // console.log("etape2)");
        // // Insérer une demande
        // const demandeResponse = await axios.post(
        //     `${process.env.URL_WEBSERVICE}ExecStoredProc/InsertDemande`,
        //     {
        //         description: 'Demande automatique pour inscription',
        ...
        // if (!demandeResponse.data?.result?.[0]?.id) {
        //     throw new Error('Échec de la création de la demande : ID non retourné');
        // }
        // const demandId = demandeResponse.data.result[0].id;

        // await axios.post(
        //     `${process.env.URL_WEBSERVICE}ExecStoredProc/InsertDemandeUsers`,
        //     {
        //         id_demande: demandId,
        ...
        return res.render('connexion', { errors }); // Changé de 'connexion' à 'register'
    }
});
...
app.get('/dashboard', noCache, async (req, res) => {
  ...
  if (!req.session.id_demandeur) {
    // console.error('Erreur : id_demandeur est indéfini. Redirection vers la page de connexion.');
    return res.redirect('/connexion');
  }
  let demandes = [];
  try {
...
const {
 ...
 if (id_demande) {
    ...
    if (id_demande) {
      ...
      const sejourValues = ['NULL', 'NULL', id_demande, 'NULL', 'NULL', "'Non renseigné'", 'NULL', 'NULL', 'NULL'];
...
app.post('/deleteDemande', async (req, res) => {
  try {
    const { id_demande } = req.body;
    if (!id_demande) {
      return res.status(400).json({ error: 'Le champ id_demande est requis.' });
    }
    const deleteTab = ['MOTIF_E_DEMANDE', 'SEJOUR_DEMANDE', 'DEMANDE_PIECE', 'DEMANDE_PATIENT'];
    for (const table of deleteTab) {
      await axios.get(
        `${process.env.URL_WEBSERVICE}deleteFromDB/${table}/DEMAND_ID=${id_demande}`,
        {
          headers: {
            'Authorization': 'Basic ' + Buffer.from(`${process.env.SQL_USER}:${process.env.SQL_PASSWORD}`).toString('base64')
          }
